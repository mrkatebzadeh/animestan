...
use crate::{AppConfig, EpisodeTracker};
...
/// Plays an episode via the configured player while updating playback state.
///
/// # Errors
///
/// Returns `Err` when the configured player cannot be spawned or when watched
/// state updates fail before playback starts.
pub fn play_episode(
    config: &AppConfig,
    ...
    play_episode_inner(config, tracker, episode_id, stream_url, output)
}
...
fn configure_stdio(command: &mut Command, output: PlayerOutput) {
...
mod tests {
    ...
    #[test]
    fn player_command_defaults_to_mpv() {
    ...
    #[test]
    fn player_command_splits_binary_and_args() {
        let config = AppConfig {
            player: Some("vlc --fullscreen".to_string()),
            ..AppConfig::default()
        };
        let (binary, args) = player_command(&config);
    ...
    #[test]
    fn mpv_binary_detection_uses_file_name() {
        assert!(is_mpv("mpv"));
        assert!(is_mpv("/usr/local/bin/mpv"));
        assert!(!is_mpv("vlc"));
    }
}
